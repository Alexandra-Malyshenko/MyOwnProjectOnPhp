...
class WishListService
{
    ...
    {
        $wishList = $this->wishListRepos->getByUserId($user_id);
        $productsList = [];
        foreach ($wishList as $item) {
            $product = (new ProductRepository())->getById($item->getProductId());
    ...
    {
        return $this->wishListRepos->getById($id);
    }

    public function createWish($product_id, $user_id): bool
    {
        return $this->wishListRepos->create($product_id, $user_id);
    }

    public function updateWish(int $id, $product_id, $user_id): bool
    {
        return $this->wishListRepos->update($id, $product_id, $user_id);
    }

    public function delete(int $id): bool
    {
        return $this->wishListRepos->delete($id);
    }

    public function count(int $id): int
    {
        return count($this->wishListRepos->getByUserId($id));
    }
}